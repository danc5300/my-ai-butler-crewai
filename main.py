...
Keep it warm, personal and high energy."""
...
def handle_message(message):
    ...
    if user_id not in memory:
        ...
        bot.reply_to(message, "Welcome! I'm Blaze (and Alfred is here too). Type /help for commands.")
    ...
    if text == "/help":
        ...
        return
    if text == "/upgrade":
        bot.reply_to(message, "🔼 Upgrade here:\nEssential ($29): [Your Polar Link]\nPremium: [Your Polar Link]")
        return

    if daily_count >= limit:
        bot.reply_to(message, f"You've hit your limit ({limit} messages). Type /upgrade to get more!")
        return
