...
def root():
    return {
        ...
        "alfred": "Ready (formal butler - main)",
        "blaze": "Ready (spicy mode - shared memory)",
        "content_creation": "Enabled - YouTube scripts, Instagram Reels, Facebook posts",
        "memory": f"{len(shared_memory)} items in shared memory"
    }

@app.post("/add_memory")
def add_memory(message: str):
    shared_memory.append({"message": message, "timestamp": "now"})
    return {"status": "added to shared memory"}
